...
extension Diff {
    ...
    public struct File: LibGit2RawRepresentable {
        ...
        init(raw: git_diff_file) {
            ...
            size = Int(raw.size)
            flags = Flag.from(_u32(raw.flags))
            mode = FileMode(raw: git_filemode_t(rawValue: numericCast(raw.mode)))
...
extension Diff {
    ...
    public enum Flag: Sendable {
        ...
        static func from(_ flags: UInt32) -> [Flag] {
            ...
            if flags & _u32(GIT_DIFF_FLAG_BINARY.rawValue) != 0 {
                result.append(.binary)
            }

            if flags & _u32(GIT_DIFF_FLAG_NOT_BINARY.rawValue) != 0 {
                result.append(.notBinary)
            }

            if flags & _u32(GIT_DIFF_FLAG_VALID_ID.rawValue) != 0 {
                result.append(.validID)
            }

            if flags & _u32(GIT_DIFF_FLAG_EXISTS.rawValue) != 0 {
                result.append(.exists)
            }

            if flags & _u32(GIT_DIFF_FLAG_VALID_SIZE.rawValue) != 0 {

...
/// A dependency-free SHA-256 implementation (FIPS 180-4).
///
/// Package integrity must work identically on macOS, Linux, Windows and WSL and
/// inside the iOS sandbox, so swiftbox does not lean on CryptoKit (Apple-only)
/// or any system crypto library. This is a straightforward, self-contained
/// implementation used to verify downloaded sources against the
/// `TERMUX_PKG_SHA256` recorded in each recipe.
public enum SHA256 {
    ...
    /// Lowercase hex digest of `string`'s UTF-8 bytes.
